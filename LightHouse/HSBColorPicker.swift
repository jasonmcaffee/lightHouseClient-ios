...
//  LightHouse
//
//  Created by Jason McAffee on 9/20/16.
...
class HSBColorPicker : UIControl {
    ...
    private func initialize() {
        ...
        addGesture(self, action: #selector(HSBColorPicker.touchedColor(_:)))
    }
    
    //action:(SEL)action forControlEvents:(UIControlEvents)controlEvents
    func addGesture(view: UIView, action:Selector){
        let touchGesture = UILongPressGestureRecognizer(target: view, action: action)
